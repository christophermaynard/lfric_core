...
module basis_function_mod
  ...
  !> 2-dim allocatable arrays of reals which hold the values of the nodal
  !> co-ordinates for the W2V function space
  real(kind=r_def), allocatable :: w2v_nodal_coords(:,:)
  !> 2-dim allocatable arrays of reals which hold the values of the nodal
  !> co-ordinates for the W2H function space
  real(kind=r_def), allocatable :: w2h_nodal_coords(:,:)
  ...
  integer, allocatable :: w2v_dof_on_vert_boundary(:,:)
  integer, allocatable :: w2h_dof_on_vert_boundary(:,:)
  ...
  integer, allocatable :: w0_basis_order(:,:), w0_basis_index(:,:), &
                          ...
                          w2v_basis_order(:,:), w2v_basis_index(:,:), &
                          w2h_basis_order(:,:), w2h_basis_index(:,:)
  ...
  real(kind=r_def), allocatable :: w0_basis_vector(:,:), w0_basis_x(:,:,:), &
                                   ...
                                   w2v_basis_vector(:,:), w2v_basis_x(:,:,:), &
                                   w2h_basis_vector(:,:), w2h_basis_x(:,:,:)

  integer :: w0_order, w1_order, w2_order, w3_order, wtheta_order, w2v_order, w2h_order
...
contains 
  ...
  subroutine get_basis(k, &
    ...
    ! order of elements
    integer, intent(in) :: k
    integer, intent(in) :: w_unique_dofs(7,4), w_dof_entity(7,0:5)
    ...
    allocate(unit_vec_w2v(w_unique_dofs(6,2),3))
    allocate(unit_vec_w2h(w_unique_dofs(7,2),3))
   ...
   allocate(w2v_basis_x(k+2,3,w_unique_dofs(6,2)))
   allocate(w2h_basis_x(k+2,3,w_unique_dofs(7,2)))
   ...
   w2h_order = k
    ...
    do idx=1,w_unique_dofs(6,2)
      do i=1,3
        unit_vec_w2v(idx,i) = 0.0_r_def
      end do
    end do
    ...
    do i=nfaces-1,nfaces
      do j1=1,k+1
        do j2=1,k+1 
          ...
          ly(idx) = j(j2l_face(i,2))
          lz(idx) = j(j2l_face(i,3))
          unit_vec_w2v(idx,:) = normal_to_face(i,:)
          if (i == nfaces - 1) w2v_dof_on_vert_boundary(idx,1) = 0
          if (i == nfaces )    w2v_dof_on_vert_boundary(idx,2) = 0
          idx = idx + 1
        end do
      end do
    end do
    ...
    w2v_basis_index(2,:) = ly(1:w_unique_dofs(6,2))
    w2v_basis_index(3,:) = lz(1:w_unique_dofs(6,2))
    ...
    do idx=1,w_unique_dofs(7,2)
      do i=1,3
        unit_vec_w2h(idx,i) = 0.0_r_def
      end do
    end do
    ...
    do i=1,nfaces-2
      do j1=1,k+1
        do j2=1,k+1 
          ...
          ly(idx) = j(j2l_face(i,2))
          lz(idx) = j(j2l_face(i,3))
          unit_vec_w2h(idx,:) = normal_to_face(i,:)
          if (i == nfaces - 1) w2h_dof_on_vert_boundary(idx,1) = 0
          if (i == nfaces )    w2h_dof_on_vert_boundary(idx,2) = 0
          idx = idx + 1
        end do
      end do
    end do
    ...
    w2h_basis_index(2,:) = ly(1:w_unique_dofs(7,2))
    w2h_basis_index(3,:) = lz(1:w_unique_dofs(7,2))
    ...
    deallocate ( unit_vec_w2, unit_vec_w1, unit_vec_w2v, unit_vec_w2h)
